...
public interface PaymentsRepo extends JpaRepository <Payments, Long> {
    ...
    @Query(value = "SELECT * FROM money_tracker_appsql.payments WHERE CATEGORIES = :categories AND customer_id = :customerId order by purchase_date desc Limit 1", nativeQuery = true)
